<template>
	<header class="main_header" role="banner" id="mainHeader">
			...
			<div class="container">
				<div class="main_header__main_inner">
					...
					<div class="main_header__right" v-if="isAuthenticated">
						...
						<nuxt-link
							to="/create-listing"
							class="btn btn__secondary create_listing_btn"
							...
							<ul role="notifications">
								...
								<li :class="{ 'is_active' : activeNotification === 2 }">
									...
									<div class="list_sub">
										...
										<button v-if="seeAllNotifications" class="see_all" @click="toggleSeeAllNotifications()" title="see less">See less</button>
										<button v-else="seeAllNotifications" class="see_all" @click="toggleSeeAllNotifications()" title="see all">See all</button>
									</div>
								</li>
							</ul>
						</div>
		...
		<no-ssr>
			...
			<modal v-if="showModalEmailCode" class="modal_login user_modal modal_email_code" @close="closeModalEmailCode()">
				<div slot="body">
					<div class="main_logo">
						<img src="~/assets/img/logo-svg.svg" alt="main logo"><i>Tradelist</i>
					</div>
					<p>Enter Validation Code</p>
					<form method="post" @submit.prevent="activate" class="email_code" :class="{'form_invalid' : errorActivateCode && !resendActivationEmail}">
						<div class="form_group">
							<input
								type="text"
								required
