				...
				<div class="fieldset_wrapper fieldset_offer">
					<fieldset>
						<div class="breadcrumb">
							<ul>
								<li>
									<nuxt-link :to="{name: 'dashboard-trade-history'}">Trade History</nuxt-link>
								</li>
								<li v-if="loggedInUser.id == currentTrade.listing.user.id">
									<div
										class="product_image"
										:style="{ backgroundImage: 'url('+ listingMainPhoto(currentTrade.listing) + ')' }"
									></div>
								...
								<li v-else-if="tradePhoto(currentTrade)">
									<div
										class="product_image"
										:style="{ backgroundImage: 'url('+ tradePhoto(currentTrade) + ')' }"
									></div>
							...
							<tbody>
								<tr v-for="(data, index) in trades" :key="index">
									<td>
										<div
											class="product_image"
											v-if="loggedInUser.id == data.listingUser"
											:style="{ backgroundImage: 'url('+ data.listingPhotoUrl  + ')' }"
										></div>
										<div
											class="product_image"
											v-else-if="data.itemPhoto"
						...
						<div class="card_wrapper">
							<div class="card">
								...
								<ul>
									<li>
										<nuxt-link
											:to="{name: 'listing-details-id',params: {id:currentTrade.listing.id} }"
											target="_blank"
										>
											<div
												class="product_image"
							...
							<div class="card">
								...
								<ul>
									<li v-for="data in currentTrade.tradeOffers">
										<nuxt-link
											:to="{name: 'listing-details-id',params: {id:data.listing.id} }"
											v-if="data.listing"
											target="_blank"
										>
											<div
												class="product_image"
												:style="{ backgroundImage: 'url('+ listingMainPhoto(data.listing) + ')' }"
											></div>
											<p>
												{{data.listing.title}}
...
<script>
	...
	export default {
		...
		async asyncData({ store, params }) {
			...
			return { trades: trades, currentTrade: currentTrade };
		},
		computed: {
