<template>
	<div class="page_create_listing">
		...
		<div class="create_listing_page_tile">
			<div class="container">
				<h1>Edit listing</h1>
			</div>
		</div>
		...
		<no-ssr>
			<modal v-if="showModalListingSuccess" class="modal_listing_success success_modal"
				...
				<div slot="body">
					...
					<p>Your {{listingData.title}} just updated.</p>
...
<script>
	...
	import StepNavigation from "~/components/listing/StepNavigation.vue";
				...
				meta: [
					{
						hid: "description",
						name: "description",
						content: "Edit listing description"
					}
				]
			};
		},
		async fetch({ params, store, route }) {
			await store.dispatch("editListing/setListingAsync", params.id);
		},
		created() {
			if (Object.keys(this.listingData).length < 1) {
				this.$router.push({ name: "index" });
			}
		},
		data() {
			return {
				listingData: this.$store.getters["editListing/getData"],
		...
		methods: {
			...
			async submitListing(data) {
				...
				return await this.$store
					.dispatch("editListing/sendListing")
					.then(response => {
			...
			async updateListing(data) {
				let parent = this;
				this.listingData = await this.$store
					.dispatch("editListing/updateListing", data)
					.then(response => {
						return parent.$store.getters["editListing/getData"];
					});
			},
			resetListing() {
				this.$store.commit("editListing/resetStateData");
				this.listingData = this.$store.getters["editListing/getData"];
			}
		}
	};
</script>
